...
import org.example.app.dto.CodeRequestDto;
...
public class UserHandler {
  ...
  public void restoreCredentials(HttpServletRequest req, HttpServletResponse resp){
    log.log(Level.INFO, "restore code");

      try {
        final CodeRequestDto codeRequestDto = gson.fromJson(req.getReader(), CodeRequestDto.class);
        final var responseDto = service.restoreCredentials(codeRequestDto);
        resp.setHeader("Content-Type", "application/json");
        resp.getWriter().write(gson.toJson(responseDto));
